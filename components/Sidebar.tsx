...
import { Search, Compass, List, Filter, Calendar, Settings, User, CheckCircle, Clock } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentTab, setTab, toggleFilter }) => {
  ...
  const NavItem = ({ tab, icon: Icon, label }: { tab?: TabView; icon: any; label: string }) => {
    ...
    return (
      ...
      >
        ...
        <span className={`text-base whitespace-nowrap transition-opacity duration-200 ${isHovered ? 'opacity-100 delay-75' : 'opacity-0'}`}>
          {label}
        </span>
      </button>
    );
  ...
  return (
    <div 
      className={`fixed left-0 top-0 h-screen bg-[#f3f4f6] z-50 flex flex-col justify-between py-6 transition-all duration-300 ease-in-out border-r border-gray-200 shadow-lg ${isHovered ? 'w-64' : 'w-20'}`}
    ...
    >
      <div>
        ...
        <div className="flex items-center h-16 mb-6">
           ...
           <span className={`text-2xl font-bold text-gray-800 tracking-tight transition-opacity duration-200 ${isHovered ? 'opacity-100 delay-75' : 'opacity-0'}`}>
             trakt
           </span>
        </div>
      ...
      <div className="space-y-4">
        <div className={`flex items-center mx-3 rounded-xl hover:bg-gray-200 transition-colors cursor-pointer p-2 ${isHovered ? '' : 'justify-center'}`}>
